...
describe "Creating a new support form" do
  ...
  it "creates a new form", js: true do
    ...
    expect(page).to have_selector("input[name^=support_form_stat\\[category_name\\]]")
    expect{
      click_link("add_category")
      }.to change{ all("input[type=text]").count }.by(1)
    fill_in "support_form_stat_category_name", with: "Payment"
    fill_in "support_form_stat_category_name_1", with: "Replacing wrong image"
  ...
  describe "Editing a Support Form", js: true do
    before(:each) do
      @support_form_stats = SupportForm::Stat.create(stats: {"Q1" => 0, "Q2" => 0}, "recipient_email"=>"e@e.com")
    end
    ...
    it "edits and update form categories", js: true do
      ...
      find_field("support_form_stat_category_name_0").set("A1")
      expect{
        click_button "Edit Categories"
        expect(page).to have_content ""
      }.to change{ @support_form_stats.reload.stats.first.first }.to("A1")
